function ExtractAvgSlowingDownTimeFromJFile(pathOfDirectoryContainingJFile,jFileName,foutput,strFilterPageNo,VariableName)
...
display('ExtractAvgSlowingDownTimeFromJFile() is working. ;-)')
...
if exist(tempFile,'file')~=0
    sprintf('%s already exists in %s.\nThe function ExtractAvgSlowingDownTimeFromJFile is skipped.',foutput,jFileName)
    return
end
...
%             tline0
            checkVariable = 1;
            t1 = str2num(tline0(44:54)); % slowdown time
        end
    end
    ...
    if checkVariable == 1 % save data with new format
        ...
        checkVariable = 0;
        if counter == 1
            fprintf(fout,'  TIME (s)      slowdown t. (s)\n');
        end
        fprintf(fout,'%10.4f       %10.4e\n',tt,t1);
    end
end
fclose(fid);
fclose(fout);
